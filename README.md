...
- **📲 真机直连安装**：连接 USB 数据线即可自动识别设备规格与系统版本，安装前自动检查 IPA 完整性、签名结构、最低系统要求和设备类型兼容性。
- **👥 多账号灵活切换**：支持多 Apple ID 登录与一键无缝切换，支持按账号独立管理、清理凭据，并具备本地钥匙串密码保护。
- **✅ 可靠下载校验**：支持安全断点续传，并通过下载哈希与 ZIP CRC 校验避免使用损坏的 IPA。
- **🔄 自动检查更新**：启动时每日检查一次 GitHub Releases，也可在“关于软件”中一键手动检查最新发布版本。
- **📁 账号隔离归档**：下载的 IPA 安装包会自动按当前登录的 Apple ID 独立建立文件夹归档（`data/downloads/账号标识/`），井井有条。
- **🎨 纸间手绘与现代极简双风格**：采用同源实心九宫格手绘美学与现代 Apple 极简两种设计风格，支持自由切换，侧边栏导航开阔自然。
...
### 5. 设备直装
通过 USB 数据线直连 iOS 设备，支持拖放 IPA 安装包，自动完成 IPA 完整性、签名结构、最低系统版本与设备类型兼容性检测，一键安全传输安装。
![设备直装](docs/images/05_installer.png)

---

### 6. 账号中心
安全便捷的 Apple ID 登录管理，支持多账号列表展示与一键无缝切换，提供按账号独立清理凭据、刷新状态，并具备本地钥匙串密码保护与双重认证（2FA）秒级确认。
...
### 8. 关于软件
包含软件版本、检查更新、开源信息以及官方 GitHub 仓库直达链接，支持启动时自动检查与一键手动检测新版本。
...
- **设备驱动**：已安装 Apple 设备通信驱动（可通过电脑安装官方 iTunes 或 Apple Devices 获得）
